...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pretrained_model_name_or_path", type=str, default=DEFAULT_SD2_MODEL_ID,
    ...
    parser.add_argument("--finetuning_method", type=str, default="xattn",
                        choices=["full", "selfattn", "xattn", "noxattn", "notime", "selfattn+xattn", "k+v",
                                 "xattn_out"])
    ...
    parser.add_argument("--exp_name", type=str, default="SDDsvd")
...
def gather_parameters(args: argparse.Namespace, unet: UNet2DConditionModel) -> Tuple[
    List[str], List[torch.nn.Parameter]]:
    ...
    for n, p in unet.named_parameters():
        if args.finetuning_method == "full":
            ...
            names.append(n)
            parameters.append(p)
        elif args.finetuning_method == "selfattn+xattn":
            # Attention layer 1 is the self-attention layer.
            if "attn1" in n or "attn2" in n:
                if "delta" in n:
                    ...
                    names.append(n)
                    parameters.append(p)
        elif args.finetuning_method == "k+v":
            # Attention layer 1 is the self-attention layer.
            if "attn2" in n:
                if "to_k" in n or "to_v" in n:
                    if "delta" in n:
                        ...
                        names.append(n)
                        parameters.append(p)
        elif args.finetuning_method == "xattn_out":
            # Attention layer 2 is the cross-attention layer.
            if "attn2" in n and "to_out" in n:
                if "delta" in n:
                    p.requires_grad = True
                    names.append(n)
                    parameters.append(p)
...
def save_checkpoint(
    ...
    if step is not None:
        ...
        state_dict = {k: v for k, v in unet.state_dict().items() if "delta" in k}
        save_file(state_dict, os.path.join(output_dir, "spectral_shifts.safetensors"))
    else:
...
def encode_prompt(
    ...
    if removing_prompt is not None and isinstance(removing_prompt, str):
        ...
        assert len(prompt) == len(
            removing_prompt), f"Safety concept must be the same length as prompt of length {len(prompt)}."

    if negative_prompt is not None and isinstance(negative_prompt, str):
        negative_prompt = [negative_prompt]
        assert len(prompt) == len(
            negative_prompt), f"Negative prompt must be the same length as prompt of length {len(prompt)}."
...
def sample_until(
    ...
    for i, t in enumerate(timesteps):
        ...
        # print("原始latent_model_input的设备:", latent_model_input.device)
        # latent_model_input = latent_model_input.to(devices[0])
        # print("强制指定到devices[0] latent_model_input的设备:", latent_model_input.device)
        # print("prompt_embeds的设备:", prompt_embeds.device)
        # prompt_embeds = prompt_embeds.to(devices[0])
        # print("强制指定到devices[0] prompt_embeds的设备:", prompt_embeds.device)

        # predict the noise residual
        unet = unet.to(devices[0])
        noise_pred = unet(latent_model_input.to(devices[0]), t,
                          encoder_hidden_states=prompt_embeds.to(devices[0])).sample
...
def train_step(
    ...
    with torch.no_grad():
        ...
        e_0_old = unet_teacher(_latents, t_ddpm.to(devices[1]), encoder_hidden_states=uncond_emb).sample
        e_0_old = e_0_old.detach().to(devices[0])

    e_p_new = unet_student(latents, t_ddpm.to(devices[0]), encoder_hidden_states=safety_emb.to(devices[0])).sample

    loss = F.mse_loss(e_p_new, e_0_old)
    return loss
...
def main():
    ...
    unet_teacher = load_unet_for_svdiff(
        args.pretrained_model_name_or_path, subfolder="unet", revision=args.revision, variant=args.variant,
    )
    unet_student = load_unet_for_svdiff(
        args.pretrained_model_name_or_path, subfolder="unet", revision=args.revision, variant=args.variant,
    )
    ...
    step = 0
    if args.eval_at_first and args.validation_prompts:
        validate(
            args=args,
            vae=vae,
            text_encoder=text_encoder,
    ...
    for step in progress_bar:
        ...
        if (step % args.log_every == 0) and (args.logging_dir is not None):
            ...
            plot_loss(losses, f'{args.logging_dir}/loss.png', prompt, n=3)
        # Validation
        if (step % args.eval_every == 0) and (step >= args.eval_after) and args.validation_prompts:
            validate(
                args=args,
                vae=vae,
                text_encoder=text_encoder,
